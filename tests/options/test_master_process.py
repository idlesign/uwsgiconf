...
def test_master_process_basics(assert_lines):
    ...
    ], Section().grp_master_process.attach_process('/here/a.pid', background=False))

    assert_lines([
        'smart-attach-daemon = /here/a.pid',
    ], Section().grp_master_process.attach_process('/here/a.pid', background=True))

    with pytest.raises(ConfigurationError):
        assert_lines([
            '',
        ], Section().grp_master_process.attach_process('command', background=True))

    assert_lines([
        'attach-daemon = command',
    ], Section().grp_master_process.attach_process('command', background=False))

    assert_lines([
        'attach-control-daemon = command',
    ], Section().grp_master_process.attach_process(
