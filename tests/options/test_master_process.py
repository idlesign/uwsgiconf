...
def test_master_process_basics(assert_lines):
    ...
    ], Section().master_process.attach_process('/here/a.pid', background=False))

    assert_lines([
        'smart-attach-daemon = /here/a.pid',
    ], Section().master_process.attach_process('/here/a.pid', background=True))

    with pytest.raises(ConfigurationError):
        assert_lines([
            '',
        ], Section().master_process.attach_process('command', background=True))

    assert_lines([
        'attach-daemon = command',
    ], Section().master_process.attach_process('command', background=False))
